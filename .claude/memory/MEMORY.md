...
- ⭐⭐ **`DEC-142`→`DEC-146` (2026-09-07): `ADR-0046` APPROVED AND SHIPPED — THE FTS IMAGE IS BACK ON THE 2022 LINE; `DEF-130`/`DEF-121` FIXED ON A CLEAN POST-SHIP FALSIFIER** (`sql-startup-sampling.yml`, `arm`×`contention`; arms now `fts-shipped`/`stock-shipped-base`). 2025 line 47/660 vs 2022 line 0/1260 (p=1e-21). ⛔ A 17.x-booted `/var/opt/mssql` volume KILLS the 2022 image (3/3). ⚠ `MSYS_NO_PATHCONV=1` for AWS paths in Git Bash. **Delegated verdicts ("verify, test and feedback") ⇒ verify from ARTEFACTS with a control.** ⛔ **`DEF-145`: `WBS-28`/`SL-037` were flipped by the AGENT with NO ruling — sweep the decision AND progress registers for the id before calling a status the operator's** (`DEC-147` d3 supplied it). ⭐ `LL-070`: when you retract, grep the PR's DIFF for the retracted PHRASE. ⭐ `DEC-148`: `LL-067`–`LL-070` PINNED, `SL-035` Implemented, next = DW review round. ⭐⭐⭐ **`DEC-149` (09-08): THE AGENT NOW DOES CLOUD ACTS — `DEF-143` CLOSED, both boxes re-pushed via SSM, `SL-040`/`SL-042` Implemented.** ⛔⛔ **`DEF-147`/`DEF-148`: `promote-image.sh` logged SUCCESS while aliasing a SECOND image object, so the pinned tag stayed on the count clock. READ THE REGISTRY, NEVER THE EXIT CODE.** ⚠⚠ **`simulate-principal-policy` does NOT populate `aws:CurrentTime`, so a date-conditioned grant reads `implicitDeny` while LIVE — pass `--context-entries` or your revoke proof is worthless.** ⚠ Both instance roles are PULL-ONLY; a push needs a scoped self-expiring grant. ⚠ Git Bash `mktemp` gives `/tmp/…`, invisible to the Windows AWS CLI. ⭐ A re-pushed image keeps its CONFIG digest, not its manifest digest; ⛔ `DEF-146`: the watchdog's FACTORY WIRING is unguarded — mutate the REGISTRATION, not only the code it registers (M1 13/13 rebuilt, M2 12/1). Resolve state from `DEC-148` onward, never here.

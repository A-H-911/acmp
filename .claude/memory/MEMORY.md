...
⛔⛔⛔ **ALL PACKAGE READS *AND* WRITES GO THROUGH THE `tamheed` MCP TOOLS — NO EXCEPTIONS** (`DEC-135` d1, an operator OVERRIDE). **Never open/`cat`/`grep`/`wc -l` a `tamheed-package/data/*.jsonl`** — 78 ad-hoc scripts were deleted for it and an integrity audit followed (engine clean, no data manipulation, `PE-897`/`PE-898`). ⛔ **The committed `gen-*.mjs` slates AND `count-prompt-ids.py` ALL read the JSONL ⇒ running one is NON-COMPLIANT** — `DW-100`, the rebuild, is the **next session's first act**, then `skill-promote.md` (`PE-909`).
⭐⭐ **THE TOOL IS NOW SUFFICIENT (4.5.0/4.6.0): `entity_query` takes `after_id`/`ids`/`search`, returns `total`, and TRUNCATES NO FIELD** (read in source). ⛔⛔ **TRAP 13 IS REVERSED — build payloads FROM `entity_query`, never the JSONL**; `LL-045` is pinned on the refuted premise and needs supersession. ⭐ New too: `package_verify` (`foreign: []`), `amends`+`retire` edges, `audit_evidence` narrated/ungraded split, `lessons-note-budget`. ⚠ **JSONL flushes on EVERY write, not on `package_close`** — `AGENTS.md` is wrong (measured).

...
- **★ NEXT SESSION: execute `~/.claude/plans/the-3-of-the-abstract-ember.md`** — approved 9-day plan for §3 of the resume file. **Day 1 item 1 is DONE** (PR #221, `AC-085` leg 1 observer). **Next up: D-A, the audit-log pollution** — `ProvisionCurrentUser.cs:63` emits `ProfileSynced` **unconditionally**, so ~14 rows landed in one minute of browsing, into an **immutable** chain. **Diagnose the caller first** (per-navigation or polling? unproven), then fix the handler to emit only on a real change.

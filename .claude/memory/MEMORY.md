...
- ⭐⭐ **THE MANDATED VERDICT ROUTE WORKS END TO END** (`DEC-141` d1): export EVERY family in ONE pass → `package_verify()` digest == exports' digest → `gen-slice-review-slate` (a slice with *nothing at Review* refusing = the CONTROL) → operator rules → full-row flips with `expect_unchanged` on `title, phase_id, slice_id, effort, source_kind, source_span`. ⚠ **An OMITTED guarded column REFUSES THE WHOLE BATCH.** ⭐ **`gen-record-slate` indexes EVERY file in `exports/` ⇒ delete stale exports of OTHER families first.** ⛔ Offer no verdict and no "(Recommended)" on the operator's own questions (`DEC-140` d3).
- ⭐⭐ **`DEC-142`→`DEC-146` (2026-09-07): `ADR-0046` APPROVED AND SHIPPED — THE FTS IMAGE IS BACK ON THE 2022 LINE; `DEF-130`/`DEF-121` FIXED ON A CLEAN POST-SHIP FALSIFIER** (`sql-startup-sampling.yml`, `arm`×`contention`; arms now `fts-shipped`/`stock-shipped-base`). 2025 line 47/660 vs 2022 line 0/1260 (p=1e-21); `heavy` delivered when `backstop` gave 0/120 ⇒ key a control on what DELIVERS. ⛔ **A 17.x-booted `/var/opt/mssql` volume KILLS the 2022 image** (3/3, core dump) — classify every volume; prod/uat pins are pre-move (SSM `/acmp/<env>/env`; `MSYS_NO_PATHCONV=1` or `ParameterNotFound`). ⚠ **`DEF-143`: ECR keep-last-10 EXPIRED every pinned image** (`LL-069`; re-push = OPERATOR's act). ⭐ `LL-067` control=the COUNT; ⛔ `LL-068` `0 0` before `checkout -b`. **Operator delegates verdicts in words ("verify, test and feedback") ⇒ verify from ARTEFACTS with a control, never from rows.** ⛔ **`DEF-145`: `WBS-28`/`SL-037` were flipped by the AGENT with NO ruling — sweep the decision AND progress registers for the id before calling a status the operator's** (`DEC-147` d3 supplied it). ⭐ `LL-070`: when you retract, grep the PR's DIFF for the retracted PHRASE. Resolve state from `DEC-147` onward, never here.
...
⭐⭐ **`entity_query` TRUNCATES NO FIELD** (`tamheed_server.py:993-1076`, read in source, `PE-910`) — TRAP 13 REVERSED; `LL-045`→`LL-063`. ⭐⭐⭐ `DW-100`/`DEC-135` d1 called it UNTESTABLE because both framed the instruments as OUTPUTS to compare, so the CODE producing one was invisible — ask that whenever a premise looks circular. ⭐⭐ `trg_lessons_immutable` ABORTS content drift on an `Approved` lesson ⇒ a supersession paste is SELF-VERIFYING; `DEC-137` d4 (`decision_ref` == `amends` target ⇒ provenance) refused my `SC-051` edge. ⛔ `deferred_work`/`wbs_items`/`scope_changes` have NO trigger (`DEF-127`) ⇒ use 4.7.0 `expect_unchanged: [cols]` — an OMITTED named column counts as CHANGED; CALIBRATE it (a refused `effort` M→L rolled the batch back). ⭐ Omit a nullable column and it is preserved untouched.
- ⭐⭐⭐ **`DEC-140`: A GREEN `lessons-confirmed` CAN MEAN *NOTHING WAS RECORDED*** — it counts `Proposed` rows, so it goes green precisely when you filed no lesson. ⭐ **`entity_query("lesson", status="Proposed")` + a `Promoted` control is the real check.** `LL-066`: sweep prose BEFORE committing a ruling, keyed on the SHAPE of absence/presence claims — `no |none|nothing|names no|not scheduled|deliberately|standing|exists` — read for what each hit DENIES or ASSERTS (caught AGAIN 2026-09-07 in `prm-next.md` item 1); `LL-065`: a PROVEN premise argues FEASIBILITY, never DESIRABILITY; `DW-101`: two readiness gates pass because their SUBJECT is missing. ⭐ **`SL-035` (status `Approved`) PASSES `readiness_check` since `DEF-121`/`DEF-130` closed (2026-09-07) — its flip is the OPERATOR's.** ⚠ **`last_referenced` moves on `work_bind`, NOT on a status flip** (`WBS-27.2` flipped 09-04, column reads 09-03) — to list what flipped in a window, use the progress register + the commit log, never that column.

...
- ⭐⭐⭐ **`DEC-140`: A GREEN `lessons-confirmed` CAN MEAN *NOTHING WAS RECORDED*** — it counts `Proposed` rows, so it goes green precisely when you filed no lesson. ⭐ **`entity_query("lesson", status="Proposed")` + a `Promoted` control is the real check.** `LL-066` (supersedes `LL-064`: sweep prose BEFORE committing a ruling, keyed on the SHAPE of absence/presence claims — `no |none|nothing|names no|not scheduled|deliberately|standing|exists` — read for what each hit DENIES or ASSERTS; caught AGAIN 2026-09-07 in `prm-next.md` item 1) and `LL-065` (a PROVEN premise argues FEASIBILITY, never DESIRABILITY) Approved; `DW-101` files two readiness gates that pass because their subject is missing (5 defects w/ null `found_in`, `AC-109`–`112` unbound). ⭐ **`SL-035` (status `Approved`) PASSES `readiness_check` since `DEF-121`/`DEF-130` closed (2026-09-07, only medium advisories) — its flip is the OPERATOR's, surfaced in the DEC-146 recap.**

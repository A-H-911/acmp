...
- ⭐⭐ **`DEC-142`→`DEC-146` (2026-09-07): `ADR-0046` APPROVED AND SHIPPED — THE FTS IMAGE IS BACK ON THE 2022 LINE; `DEF-130`/`DEF-121` FIXED ON A CLEAN POST-SHIP FALSIFIER** (`sql-startup-sampling.yml`, `arm`×`contention`; arms now `fts-shipped`/`stock-shipped-base`). One pair each: alone 0/120 vs backstop 4/120; stock 2025 w/o FTS crashed; 2025 line 47/660 vs 2022 line 0/1260 (p=1e-21); `heavy` delivered when `backstop` gave 0/120 ⇒ key a control on what DELIVERS. ⛔ **A 17.x-booted `/var/opt/mssql` volume KILLS the 2022 image** (3/3, core dump) — classify every volume; prod/uat pins are pre-move (SSM `/acmp/<env>/env`; `MSYS_NO_PATHCONV=1` or `ParameterNotFound`). ⚠ **`DEF-143`: ECR keep-last-10 EXPIRED every pinned image** (`LL-069`; re-push is the OPERATOR's act on the box). ⭐ `LL-067` control=the COUNT; ⛔ `LL-068` `0 0` before `checkout -b`. **Operator delegates verdicts in words ("verify, test and feedback") ⇒ verify from ARTEFACTS with a control, never from rows.** Resolve state from `DEC-146` onward, never here.
...
- ⭐⭐⭐ **`DEC-140`: A GREEN `lessons-confirmed` CAN MEAN *NOTHING WAS RECORDED*** — it counts `Proposed` rows, so it goes green precisely when you filed no lesson. ⭐ **`entity_query("lesson", status="Proposed")` + a `Promoted` control is the real check.** `LL-066` (supersedes `LL-064`: sweep prose BEFORE committing a ruling, keyed on the SHAPE of absence/presence claims — `no |none|nothing|names no|not scheduled|deliberately|standing|exists` — read for what each hit DENIES or ASSERTS; caught AGAIN 2026-09-07 in `prm-next.md` item 1) and `LL-065` (a PROVEN premise argues FEASIBILITY, never DESIRABILITY) Approved; `DW-101`: two readiness gates pass because their SUBJECT is missing. ⭐ **`SL-035` (status `Approved`) PASSES `readiness_check` since `DEF-121`/`DEF-130` closed (2026-09-07) — its flip is the OPERATOR's.** ⚠ **`last_referenced` moves on `work_bind`, NOT on a status flip** (`WBS-27.2` flipped 09-04, column reads 09-03) — to list what flipped in a window, use the progress register + the commit log, never that column.

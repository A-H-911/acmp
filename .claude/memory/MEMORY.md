...
- ⚠ **PROD IS BEHIND `main`.** Prod is pinned to `1c7f2ba`; `main` is `7ffa490`. **Days 1–2 (#222–#230) are merged but NOT deployed** — invisible to the 26 users. Sequence agreed with the operator: Day 3 → start UAT → smoke + E2E there → **only then** prod.
...
- **★ NEXT: Day 3** of `~/.claude/plans/the-3-of-the-abstract-ember.md` — the operator guide, and critically the **`ec2 start/stop` commands that exist NOWHERE in the repo** (verified `wc -l` = 0). That absence is what blocks bringing UAT up, which blocks the regression, which blocks the prod deploy. **Days 1–2 are DONE and merged** (#222–#230). Defects: 41 raised, open = `DEF-012, 036, 038, 039, 041`; three of those need an operator decision, not code.
...
- ⚠ **Rename gap NOT closed: live data.** Streams are created by admins **through the UI**, so `Streams.NameAr` (and Arabic-typed titles) may still hold the old term — a **data** task no code rename reaches. Verify against prod before calling it complete.

...
- ✅ **`AC-090` MET and literally true** (#239, `ADR-0039` Approved). It was **unsatisfiable by construction**: authorization is token-driven (`IsInRole` → `ClaimsPrincipal`), so a removed role survived a full access-token lifetime (**300s, measured on the live realm**) and forced sign-out cannot revoke a token in flight. Now `RolesChangedAt` vs the token's `iat`, plus `AccessExpiresAt` and disabled — in **middleware**, fail-closed, proven by a **refused request**. ⚠ **An unknown subject must be ALLOWED**: ADR-0004 provisions the row just-in-time, so failing closed there refuses **every first login**. Tested at both levels.
- ⚠ **`SC-005`: an ADR named a seam its own tests could not reach.** `ADR-0039` specified `JwtBearerEvents.OnTokenValidated`; it worked — then the test showed the API host authenticates with a **non-JWT scheme**, so that seam is unreachable from any API test, i.e. only unit-provable = exactly the evidence `AV-134` refused. **When an ADR names a specific seam, check the harness can reach it before approving.**
- ✅ **Guest-expiry sweep** (#240) — hourly; disables past-window members locally **and** in Keycloak. `IIdentityProvider` is injected as `IEnumerable` (unregistered by default; demanding it makes the **worker unbootable**), and the member is **still disabled locally** without it. Defence in depth, **not** the enforcement.
- **★ NEXT:** (1) **`FR-159`'s writer + `/session`** — nothing sets `AccessExpiresAt` yet, and the meeting's end time is Meetings-owned so it needs a cross-module contract (ADR-0001). Only thing between `AC-092` and Met. (2) deploy with `KEYCLOAK_ADMIN_ENABLED=true`. (3) `OQ-071`'s automated probe test. (4) `up.sh`/on-prem reconcile guard. (5) `OQ-069`.

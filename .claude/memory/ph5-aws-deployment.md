...
**PH-5 "Cloud hosting & environments" — RATIFIED (2026-07-24); P20 scripts written; AWS not yet provisioned.** On 2026-07-23 the operator asked to deploy ACMP to AWS (staging/UAT + production) on EC2 + Docker Compose; on 2026-07-24 they ratified and said "defaults are fine, write the scripts." The plan (revision 2, approved after an adversarial review that caught 5 fatal defects in rev 1) lives at `C:\Users\ahammo\.claude\plans\cozy-scribbling-music.md`.

**Status now:** ADR-0034..0037 **Approved** (invariant amendments ratified), PH-5 + DEC-030 Approved, OQ-059..063 **decided via defaults** (Express both envs · us-east-1 · two web images · Webex off in UAT · worker kept split; + no RI yet, SSM not SSH). `PE-132`. **P20 landing-zone scripts WRITTEN** at `deploy/aws/` (`_common.sh` + `00`account/budget `01`network `02`s3 `03`iam `04`ecr `05`route53 + README) — idempotent, **operator-run, NOT yet executed against AWS**; bash -n clean; name-isolation verified (uat→acmp-uat-recordings, prod→acmp-prod-recordings). **Next: operator runs `deploy/aws/*.sh` (00 as root → creates acmp-admin → rest as acmp-admin), then slice P21 (`docker-compose.cloud.yml` + Keycloak→MSSQL).** PR #161 carries intake+ratification+P20.

...
- **B3** M3 wiki-local 7-icon toolbar via pure `markdownInsert.ts` util (shared MarkdownEditor untouched, 6 surfaces safe); WK8 debounced localStorage draft autosave; **WK10 = ADR-0029** replaced `TraceabilityPanel` with bespoke read-only `WikiLinkedArtifacts` card (reuses `useArtifactRelationships('Document',id)`, **drops wiki Add-relationship**); m15 muted version chip; m17 AR read-time Arabic-Indic (`ar-u-nu-arab`); m18 History ungated to all readers.

    ...
    public KnowledgeDbContext CreateDbContext(string[] args)
    {
        var options = new DbContextOptionsBuilder<KnowledgeDbContext>()
            .UseSqlServer("Server=localhost;Database=Acmp;TrustServerCertificate=True",
                sql => sql.MigrationsHistoryTable("__EFMigrationsHistory", KnowledgeDbContext.Schema))
            .Options;
        return new KnowledgeDbContext(options, new DesignTimeClock(), new DesignTimeUser());
    }

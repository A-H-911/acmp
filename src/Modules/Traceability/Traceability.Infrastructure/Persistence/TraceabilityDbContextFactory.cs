...
public sealed class TraceabilityDbContextFactory : IDesignTimeDbContextFactory<TraceabilityDbContext>
{
    public TraceabilityDbContext CreateDbContext(string[] args)
    {
        var options = new DbContextOptionsBuilder<TraceabilityDbContext>()
            .UseSqlServer("Server=localhost;Database=Acmp;TrustServerCertificate=True",
                sql => sql.MigrationsHistoryTable("__EFMigrationsHistory", TraceabilityDbContext.Schema))
            .Options;
        return new TraceabilityDbContext(options, new DesignTimeClock(), new DesignTimeUser());
    }

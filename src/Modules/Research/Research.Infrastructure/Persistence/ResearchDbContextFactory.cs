    ...
    public ResearchDbContext CreateDbContext(string[] args)
    {
        var options = new DbContextOptionsBuilder<ResearchDbContext>()
            .UseSqlServer("Server=localhost;Database=Acmp;TrustServerCertificate=True",
                sql => sql.MigrationsHistoryTable("__EFMigrationsHistory", ResearchDbContext.Schema))
            .Options;
        return new ResearchDbContext(options, new DesignTimeClock(), new DesignTimeUser());
    }

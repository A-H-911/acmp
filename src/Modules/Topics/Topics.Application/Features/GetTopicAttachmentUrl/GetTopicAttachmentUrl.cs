...
// /api/topics (GuestSurfaceMiddleware), and ITopicReader.GetMaterialUrlAsync - the same reader the guest
...
public sealed class GetTopicAttachmentUrlHandler : IRequestHandler<GetTopicAttachmentUrlQuery, string?>
{
    ...
    {
        var url = await _topics.GetMaterialUrlAsync(request.TopicId, request.AttachmentId, ct);

    ...
    public MeetingsDbContext CreateDbContext(string[] args)
    {
        var options = new DbContextOptionsBuilder<MeetingsDbContext>()
            .UseSqlServer("Server=localhost;Database=Acmp;TrustServerCertificate=True",
                sql => sql.MigrationsHistoryTable("__EFMigrationsHistory", MeetingsDbContext.Schema))
            .Options;
        return new MeetingsDbContext(options, new DesignTimeClock(), new DesignTimeUser());
    }

...
// AC-168: the graph carries stream CODES (TopicStreamReader); a row shows the stream's name.
vi.mock('../../api/members', () => ({ useStreamLabel: () => (c: string) => ({ Identity: 'Identity & Access', Payments: 'Payments & Billing' } as Record<string, string>)[c] ?? c }));
...
describe('ImpactGraphList (P10f)', () => {
  it('renders a role=tree of depth-indented rows with the focus-path crumb', () => {
    ...
    expect(screen.getByText('Identity & Access')).toBeInTheDocument(); // focus stream, by name
  ...
  it('shows a Blocked pill + stream code, and renders routeless rows as non-anchor rows', () => {
    ...
    expect(screen.getByText('Payments & Billing')).toBeInTheDocument(); // cross-stream, by name, on the blocked action

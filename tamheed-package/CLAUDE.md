...
- **LL-023** [improve, pinned] `LL-011` fixed a real failure: an artifact the operator reads to DECIDE must carry each cited record's own text, quoted from the canonical store by a generator rather than parap...

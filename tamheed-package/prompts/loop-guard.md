...
Keep this beside loop-iteration.md when running `tamheed-package` unattended. The loop stops
...
4. `readiness_check` reports a blocking failure at a slice/phase close — the
   `"force": true` override needs the operator's explicit words, so the loop halts and
   surfaces the failing rules verbatim.
...
On stop: record the reason as a final `progress_update` (this is the one write that is
always allowed), `package_close()`, and emit the ITERATION block with `stop=<reason>`

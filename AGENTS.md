...
- **Branch → reviewable PR → green CI → squash-merge → delete branch → sync main.** `main` stays green and deployable. Package, prompt and memory writes may go **direct to `main`** (both workflows path-ignore `tamheed-package/**` and `.claude/**`) — but **`scripts/**` is NOT path-ignored and is carved out: it goes via PR** (`DEC-077` d2). ⚠ **After ANY direct push to `main`, poll CI to completion** (`status` until `completed`, then `conclusion`), whatever the commit touched — a package-and-prose commit carrying one instrument runs the full pipeline, and one such commit left `main` red unnoticed (`DEF-108`).

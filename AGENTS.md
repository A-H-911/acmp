...
- **Never hand-edit `tamheed-package/` files, and never READ `tamheed-package/data/*.jsonl` either** — `DEC-135` d1 made the `tamheed` MCP tools the only path in **both** directions, with no exceptions. ⚠ **Canonical JSONL is flushed on EVERY write, not on `package_close`** — this bullet said `package_close` and that was measured false (`DW-100`; an `entity_upsert` dirties `data/*.jsonl` immediately, seen in `git status --porcelain -uall`). The distinction is load-bearing twice over: it is why `git status` must be run immediately before any branch operation rather than from a memory of having committed (`LL-061`), and it is why the JSONL was never stale for the readers that used to open it.

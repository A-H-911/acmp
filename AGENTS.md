...
**All prompts live in `tamheed-package/prompts/` — read the folder and pick.** Tamheed v3.0.0 moved handoff prompts out of the database into files, so the three that used to sit under `handoff/` are now `prm-002-initial.md` (start here), `prm-001-follow-up.md` (follow-up work) and `prm-003-review.md` (audits), alongside the ready-made operating prompts (orient-resume, progress-sync, integrity-check, generate-report, slice-review). The `handoff/` copies were deleted at cutover — they were byte-equivalent duplicates, and two sources for one prompt is how the stale one gets read. Identifier and status rules are enforced by the package schema itself.

...
- **Secretary of the Committee — أمين سر اللجنة (committee lead, primary user):** runs the backlog, agenda, meetings, minutes, follow-up. Power user; needs efficiency and bulk actions. (Role token: `Secretary`; renamed from "Coordinator". In the UI, show "Secretary of the Committee" in English and "أمين سر اللجنة" in Arabic.)
...
2. **Triage the backlog** (Secretary): review submissions → accept/return/reject → categorize, prioritize (drag or keyboard), tag streams, set dependencies/risks.
3. **Build & publish an agenda** (Secretary): pick backlog topics → order (DnD) + time-box → assign presenters → publish → notify.
4. **Run a meeting & capture minutes** (Secretary/Chairman): attendance → per-topic discussion notes → decisions → actions → review/approve minutes (versioned, immutable once published).
...
6. **Track actions** (Owner/Secretary): see my actions → update progress/evidence → get reminders → verify completion (verifier ≠ owner).
...
1. **Home / role dashboard** — role-specific cards: for Secretary (backlog health, agenda readiness, overdue actions, pending approvals); for Chairman (decisions awaiting approval, upcoming meeting, votes); for Member (my topics, my actions, my votes). Each card drills down.
2. **Backlog** — the workhorse. Multiple views: **list, table (dense, sortable, filterable, saved views), kanban (by status, DnD + keyboard), calendar, timeline**. Aging indicators, priority, owner avatars, stream tags, status chips, urgency. Bulk actions for Secretary.

...
| D-02 | Webex recording/transcript retrieval + storage | Manual-for-now | Secretary pastes link / uploads file — **manual upload + presigned playback + delete DELIVERED (P13-recording, ADR-0025, AC-073/074)**; auto-retrieval = AC-070; only transcript retrieval remains | Phase 2 (transcript → P19) | Webex Assistant enabled org-wide + API access confirmed (OQ in [open-question register](../decisions/open-question-register.md)) | In progress |

...
**Depends-On:** listed `BL-###` items must be delivered before this item starts. Items with no dependency may begin once their epic's prerequisites are met. Backlog-number gaps (BL-091–100, BL-138–200) are intentional phase-block reservations.
...
1. **Foundation first (BL-001–BL-016):** nothing else can be built without containers, migrations, config, observability, Hangfire, MinIO, and i18n/RTL scaffolding. PH-1 P1 blockers with no value-bearing features.
...
7. **Voting after decisions (BL-057 before BL-060):** votes are attached to decision records; the decision entity is the anchor.
8. **PH-2 starts with the Tarseem container (BL-101):** all diagram features, dependency graph, and Keystone diagrams depend on the render sidecar.

...
| NFR-ID | Requirement | Metric / Target | Verification Method |
|---|---|---|---|
| NFR-014 | The system shall be available **24×7** with a monthly availability target of **≥ 99.9%**, achieved via simple redundancy and nightly backups (no HA cluster, no horizontal scaling). Right-sized for on-prem, ≤20 users. | ≥ 99.9% monthly uptime; ≤ ~44 min downtime/month | Monitor via ASP.NET health check endpoint polled every 60 s; alert on 3 consecutive failures; monthly availability report from self-hosted Seq. |

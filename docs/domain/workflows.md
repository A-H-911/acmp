...
> Roles/policies reference `10-permission-role-matrix.md`; entity states reference `12-entity-lifecycles.md` (`[→ §n]`); entities reference `11-domain-model.md`. IDs/status models from `../README.md`. Every workflow's mutating step emits an `AuditEvent` (ADR-0009); notifications go via `INotificationChannel` adapters (ADR-0005). "Permissions" lists the gating policies; ABAC scope + SoD (doc 10 §E) always apply.
...
## Traceability
Implements **Deliverable 16**. Roles/policies from `10-permission-role-matrix.md`; states/transitions from `12-entity-lifecycles.md`; entities from `11-domain-model.md`; IDs/status/principles from `../README.md`. Settled decisions: ADR-0004 (onboarding), ADR-0005 (notifications/Webex adapter), ADR-0007 (Keystone research), ADR-0009/0010 (immutability/voting). Notification details in `29-notification-strategy.md`; retention/archival in `26-audit-and-records-management.md`; SLA/urgency in `09-topic-taxonomy.md`; Webex constraints in `18-webex-feasibility.md`.

...
| R-27 | **Optimistic concurrency** | Every mutable aggregate root carries `RowVersion` (SQL `rowversion`); EF Core optimistic concurrency → stale write throws `DbUpdateConcurrencyException` → API returns **409**. Chosen over last-writer-wins and over pessimistic locking. Implemented 2026-06-30 (resolves OQ-043). | ADR-0018; docs/16 §1.5 |
...
| OQ-ID | Question | Options | Recommendation | Owner | Needed-by |
|---|---|---|---|---|---|
| OQ-012 | **SPA serving: nginx vs. ASP.NET Core static files**: Should the React SPA be served by nginx (separate container) or by ASP.NET Core `UseStaticFiles()` / `MapFallbackToFile()` from the same app container? | (a) nginx: separate container; standard SPA serving practice; independent scaling if ever needed. (b) ASP.NET Core: single container; simpler Compose stack; one less image to maintain. | **Default was (b) ASP.NET Core** static file serving. **OVERRIDDEN → (a) separate nginx `web` container** (proxies `/api`→api) per user instruction 2026-06-25 + docs/34 §8 — this is the chosen v1 serving model; the build ships the `web` nginx container, not ASP.NET static files. (Recorded in `docs/_progress/ph0-validation.md`.) | Tech Lead | Resolved → (a) nginx |
...
| OQ-ID | Question | Options | Recommendation | Owner | Needed-by |
|---|---|---|---|---|---|
| OQ-034 | **Arabic FTS quality in SQL Server** (also logged as RISK-010): Is SQL Server's built-in Arabic word-breaker adequate for committee terminology (technical terms, transliterations from English)? | (a) SQL Server FTS Arabic is adequate. (b) FTS Arabic is inadequate; replace with self-hosted, app-owned OpenSearch (per ADR-0011 / R-24). | **Default: run PH-0 spike.** If ≥80% search recall on sample Arabic queries, proceed with SQL FTS. If <80%, escalate to OpenSearch behind the same search abstraction (`ISearchIndex`) in PH-2 — never the org's ELK. Do not replace proactively; validate first. | Tech Lead | PH-0 spike |
...
| OQ-041 | **Prod CI runner**: PH-1 CI uses GitHub-hosted runners; production builds for an on-prem (possibly air-gapped) VM likely need a self-hosted runner + package/registry mirrors. *(Renumbered 2026-06-25 from a proposed OQ-038 note to avoid colliding with OQ-038 = Keycloak datastore — see `docs/_progress/ph0-validation.md`.)* | (a) Self-hosted GitHub Actions runner on the prod/dev VM + NuGet/npm + container-registry mirrors. (b) Keep GitHub-hosted runners (requires outbound connectivity from the production build path). | **Default: (a) provision a self-hosted runner before prod cutover**; GitHub-hosted runners stay fine for PH-1 CI. Ties to OQ-031 (registry) + OQ-032 (NuGet/npm mirror). | DevSecOps / Org IT | P18 (prod cutover) |
| OQ-042 | **In-app user invite / provisioning vs ADR-0015**: The Administration design shows an "Invite user" / "Provision via Keycloak" flow, but ADR-0015 mandates **manual provisioning in the Keycloak admin console — no in-app account creation, no self-registration**. Should ACMP build any in-app invite affordance? | (a) Drop the in-app invite entirely; provisioning stays manual in Keycloak. (b) Render the button as a **deep-link to the Keycloak admin console** (no in-app account creation). (c) Build an in-app invite that calls the Keycloak admin API — **requires a new ADR superseding ADR-0015's "manual only" stance**. | **RESOLVED (2026-06-27): adopt (b)** — a future Administration/User-Detail slice may re-introduce the "Provision via Keycloak" affordance **only** as a deep-link/outbound link to the Keycloak admin console (target `_blank`, `rel="noopener"`), carrying NO in-app account-creation/invite form. This honors ADR-0015 (manual provisioning, no self-registration). The P4 UI refresh **removed** the design's button + invite panel; option (c) is explicitly out unless a new ADR supersedes ADR-0015. | Tech Lead (operator-adopted) | Resolved — UI lands in a future Administration slice |
| OQ-043 | **RowVersion / optimistic concurrency drift (docs/16 vs code)**: `docs/16` §1.5 states every mutable aggregate root carries `RowVersion ROWVERSION` so a stale write throws → API returns 409 (ETag-mapped). Surfaced during S5 (ADR-0016 §3): **no code carried any concurrency token** — zero `IsRowVersion` / `[Timestamp]` / `IsConcurrencyToken` across all `*.cs`, no `RowVersion` column in any migration. So the documented optimistic-concurrency backstop did not exist and could not be tested. | (a) Add `RowVersion` to mutable roots (Topic/Meeting/Agenda/Member/…) + `IsRowVersion()` config + migrations + 409 mapping, and a concurrency test — a real feature slice + an ADR. (b) Amend `docs/16` to drop the rowversion claim (accept last-writer-wins at ≤20 users, near-zero contention). | **RESOLVED (2026-06-30): adopt (a)** — implemented in the doc-integrity slice: `RowVersion` added to the built mutable roots (Topic / Meeting / Agenda / CommitteeMember) + `IsRowVersion()` config + per-module EF migrations + `DbUpdateConcurrencyException → 409` mapping in `GlobalExceptionHandler` + a Testcontainers concurrency test; recorded in **ADR-0018**. `docs/16` §1.5 is now true in code. Option (b) last-writer-wins was rejected — the operator chose to keep the documented optimistic-concurrency backstop. | Tech Lead / Architect (operator-adopted) | Resolved — ADR-0018 |
| OQ-045 | **AC-029 downstream-link gate deferred (no artifact to link yet)**: AC-029 requires that a decision cannot be **issued** until it links ≥1 downstream artifact (Action/Risk). P7a built the IssueDecision path, but the Actions and Risks modules do not exist yet (P8), so the gate is unbuildable — there is nothing to link. P7a therefore does **not** enforce AC-029; `IssueDecisionCommand` issues a recorded decision with no downstream-link check. | (a) Retrofit the gate onto the already-shipped IssueDecision path in P8 once Actions exist: add a `requires-downstream-link` precondition (validator + domain guard) checked before `Decision.Issue`, plus tests. (b) Build a placeholder link now against a non-existent artifact type (rejected — speculative, untestable). | **DEFERRED to P8 (2026-07-01)**: adopt (a). AC-029 stays `Pending` in the acceptance audit with this OQ as its tracking ref. When the Actions module lands (P8), the link-required precondition must be retrofitted onto the existing P7a `IssueDecisionCommand` (and the supersession successor's issue), with a failing-first test proving an unlinked issue is rejected. No ADR needed — this is a phase-sequencing deferral, not a design change. **RESOLVED 2026-07-02 (P8d):** gate shipped as a **handler precondition in `IssueDecisionHandler`**, NOT a `Decision.Issue` domain guard — the link count is cross-module (Actions), so the domain cannot see it; the check calls the Actions-owned `IActionLinkDirectory.DecisionHasLinkedActionAsync` (ADR-0001 seam), throws → 409 when a follow-up-bearing outcome has zero links. **Deviation from the original "domain guard" + "supersession successor's issue" phrasing (ASM-014):** the successor of a supersession is drafted+issued atomically with zero links (actions point at the *prior*), so gating it would break every correction; placing the check in the primary handler auto-exempts supersession — not a bypass, since first-issue is only reachable through the gated path, so every lineage root already passed it. Applies only to follow-up-bearing outcomes (locked P8 fork). AC-029 → **Met**. | Tech Lead / Architect (operator-confirmed) | **Resolved — P8d** |
| OQ-046 | **Dependency ↔ Relationship: write-through mirror edge vs read-time composition (P10d)**: docs/30 §1.34/§1.1 said the Dependencies module "maintains in sync" a `depends-on` `Relationship` edge for each `Dependency` — a **write-through mirror**. But `Dependency` and `Relationship` live in separate modules / DbContexts / schemas, so a mirror write is not transactional and risks dual-write drift; and the drift-free alternative (compose at read-time) needs no cross-module write port. | (a) **Read-time composition** — store the `Dependency` ONCE (Dependencies module); the traceability panel/graph query BOTH modules and merge; no mirror edge, no new `Relationship`-writer port. (b) Write-through mirror — every `Dependency` write also writes a `Relationship` row (needs a Dependencies→Traceability write seam + outbox/reconciliation; ADR-0001 cost). (c) **Unify** the two aggregates into one edge table (reverses ADR-0008 + refactors merged P10c). | **RESOLVED (2026-07-03, operator GO): adopt (a) read-time composition.** Both the architect and advisor endorsed it: it removes the only concrete harm (drift) with the least machinery and preserves the two-model boundary (docs/11 §A.3 "keep both, distinct roles"; ADR-0008/0019). Option (c) unification was explicitly considered and **rejected** (architect: sparse/nullable-column anti-pattern for the ~12 non-governed RelTypes; would fold a core bounded context into the cross-cutting Search&Traceability module or make Decisions depend on Dependencies for AC-029; different lifecycle ⇒ different aggregate). Option (b) rejected (dual-write drift + ADR-0001 cost). **Recorded via ASM-016 + docs/30 §1.1 edit** (no new ADR needed — the deviation is from a doc note, not ADR-0008's core). **Consequence:** P10f's impact traversal must UNION the Dependencies table (it does not see dependency edges via `Relationship`) — done via read-time contract composition, NOT a cross-schema recursive CTE (ADR-0020 clarifies ADR-0019's "recursive CTE" phrasing). | Tech Lead / Architect (operator GO) | **Resolved — P10d** |
| OQ-047 | **Impact-graph cross-stream: stream model for non-Topic artifacts (P10f / FR-095)**: the graph's cross-stream signal needs each node's stream, but ONLY `Topic` carries streams (`Topic.AffectedStreams` = stream codes; the master `Stream` list lives in Membership, scoping member WRITE-ABAC). Decision/Action/Risk/System/Dependency carry no stream. So a whole-graph cross-stream signal has no backing data for non-Topic nodes. | (a) **Topic-scope** — classify cross-stream only on Topic↔Topic edges (disjoint non-empty stream-code sets), via one `ITopicStreamReader` seam; non-Topic edges are never cross-stream. (b) **Inherit** — each artifact inherits its originating Topic's streams (needs per-type artifact→topic resolvers + a semantic ruling: an artifact links to zero OR many topics — union? primary?). (c) Defer FR-095 entirely (reviewers' original lean). | **DEFAULT ADOPTED (2026-07-03, operator GO): (a) Topic-scope.** Operator chose to build FR-095 now (not defer); (b) rejected for a ≤20-user tool (3–4 new seams + an unresolved multi/zero-topic semantic = over-built, #12). FR-095 recorded as **partial (Topic-scope)** in the acceptance audit, NOT "built" — in a mostly cross-TYPE governance graph the toggle lights up rarely. **This OQ stays OPEN** to track the inherit model (b), whose real undecided is the multi/zero-topic semantic, to be resolved only if the operator wants whole-graph cross-stream. Recorded in ADR-0020. | Tech Lead / Architect (operator GO) | **Default adopted (a) — P10f; inherit model OPEN** |
| OQ-044 | **Notification read-state auditing**: Should marking notifications read emit `AuditEvent`s? P6e deliberately emitted none (a notification's read-state is the recipient's own inbox state, not a governance change — mirroring single mark-read). P6b's `read-all` reconcile (NR/round-2) added an audit emit, reversing that for the bulk path. | (a) Audit nothing — read-state is personal, non-governance (the P6e stance). (b) Audit the **bulk** `read-all` sweep only — `Notifications.AllRead` after persistence; leave single mark-read un-audited. (c) Audit every read-state change (single + bulk). | **RESOLVED (2026-07-01): adopt (b)** — `read-all` emits one `Notifications.AllRead` AuditEvent after `SaveChangesAsync`, and only when ≥1 item actually flips (the `count==0` short-circuit returns before persistence → no audit noise); single `MarkNotificationRead` stays un-audited. Rationale: a one-click sweep of the entire inbox is a notable bulk action worth a record (guardrail 5 / docs/26 / ADR-0009), whereas a single mark-read is low-signal personal state. The asymmetry is intentional. This clarifies ADR-0009's scope (read-state is not generally auditable; bulk read-all is the one exception) without a new ADR. Option (a) was the P6e default; (c) was rejected as audit noise. | Secretary / Architect (operator-adopted) | Resolved — P6b (clarifies ADR-0009) |
...
- OQ-041 (prod CI runner — self-hosted runner + mirrors before prod cutover; deploy phase P18)
- OQ-042 (in-app user invite/provisioning vs ADR-0015 — **RESOLVED 2026-06-27: adopt (b) deep-link to Keycloak console, no in-app account creation**)
- OQ-043 (RowVersion / optimistic concurrency — **RESOLVED 2026-06-30: adopt (a), implemented; ADR-0018**)
- OQ-039 (future upstream Keycloak federation/brokering — deferred; ADR-0015)
- OQ-045 (AC-029 downstream-link-required-to-issue gate — **RESOLVED 2026-07-02 (P8d)**: handler precondition in `IssueDecisionHandler` via the Actions-owned `IActionLinkDirectory`; supersession auto-exempt (ASM-014); AC-029 → Met)
- OQ-046 (Dependency↔Relationship mirror vs read-time composition — **RESOLVED 2026-07-03 (P10d)**: adopt (a) read-time composition, store once, no mirror edge; unification (c) rejected; recorded via ASM-016 + docs/30 §1.1 edit; P10f must UNION the Dependencies table)
- OQ-047 (Impact-graph cross-stream stream model for non-Topic artifacts — **DEFAULT ADOPTED 2026-07-03 (P10f)**: (a) Topic-scope via `ITopicStreamReader`; FR-095 = **partial**; inherit model (b) stays OPEN, gated on the multi/zero-topic semantic; ADR-0020). Relates to OQ-018 (impact-analysis node cap — P10f sets `MaxNodes=60`, honoured with a `partial` flag).

...
| OQ-043 | **RowVersion / optimistic concurrency drift (docs/16 vs code)**: `docs/16` §1.5 states every mutable aggregate root carries `RowVersion ROWVERSION` so a stale write throws → API returns 409 (ETag-mapped). Surfaced during S5 (ADR-0016 §3): **no code carries any concurrency token** — zero `IsRowVersion` / `[Timestamp]` / `IsConcurrencyToken` across all `*.cs`, no `RowVersion` column in any migration. So the documented optimistic-concurrency backstop does not exist and cannot be tested. | (a) Add `RowVersion` to mutable roots (Topic/Meeting/Agenda/Member/…) + `IsRowVersion()` config + migrations + 409 mapping, and an S5 concurrency test — a real feature slice + likely an ADR. (b) Amend `docs/16` to drop the rowversion claim (accept last-writer-wins at ≤20 users, near-zero contention). | **Default: defer; recommend (a) as its own slice** if concurrent edits are a real risk, else (b). Not added silently in S5 (guardrail #11) — adding columns is a feature + migration, out of a test-only slice's scope. | Tech Lead / Architect | Before any slice that relies on optimistic concurrency / 409 |

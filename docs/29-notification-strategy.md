...
| Domain model | `docs/11-domain-model.md` — `Notification`, `User.Status`, `Vote.EligibleVoterIds` |
| Background jobs | README §A — app-owned Hangfire; outbox on ACMP's own SQL |
| Reporting dashboards | `docs/27-reporting-and-dashboards.md` — scheduled report delivery via in-app notifications |
| Security | `docs/25-security-controls.md` (deep-link safety, no sensitive content in payloads) |

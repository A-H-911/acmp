...
<!-- G-IDS — the AC cells below are bold ON PURPOSE; do not "tidy" it away.
     This is a DERIVED artifact (manifest.json kind=derived; naming-conventions.md §Derived): AC-### is
     DEFINED once, in acceptance-criteria.md. This file only reports verdicts on those IDs.
     The Keystone validator treats a bare ID token in a table's first column as a DEFINITION
     (validate_package.py _guess_id_column: a column where >=60% of cells fullmatch a governed ID).
     Bold breaks that fullmatch, so these stay REFERENCES. Un-bolding them re-creates 74
     duplicate-definition findings and fails the critical G-IDS gate (RESULT: NOT READY).
     Same reason security-controls-audit.md bolds its control IDs. -->

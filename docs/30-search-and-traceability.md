...
This gate is a **planning-package quality gate** (verified by the `validate_package.py` tool from Keystone), not a runtime feature. The traceability matrix is maintained in `docs/40-acceptance-criteria.md` and `docs/38-epics-and-features.md`.
...
| Domain model | `docs/11-domain-model.md` §B module map; `Relationship` aggregate in Search&Traceability module |
| IA / Navigation | `docs/14-information-architecture-sitemap.md §2.4` — "Go to Related" pattern |
| Reporting | `docs/27-reporting-and-dashboards.md` DB-12 (cross-stream dependencies), DB-23 (repeated issues via invariant links) |
| Audit | `docs/26-audit-and-records-management.md` — relationship changes are audit events |

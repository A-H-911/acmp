...
| **TB-3** | **Browser/SPA → Keycloak** (OIDC authz-code + PKCE) | User → IdP | Federated identity; MFA at IdP; roles via group/realm-role claims (ADR-0004). ACMP consumes identity, never stores passwords. |
...
|---|---|---|---|---|
| **P1** | **External unauthenticated** | None; network reach to nginx only (intranet/VPN). No self-registration. | Get in (auth bypass), or hit an unauth endpoint. | **Lower** likelihood — on-prem, no public exposure, federated auth. Still in scope for ASVS L2 baseline (auth, transport, input). |

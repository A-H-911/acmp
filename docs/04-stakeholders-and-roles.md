...
Full permission matrix in `docs/10-permission-role-matrix.md`.
...
*Traceability: Stakeholder goals and pains cross-reference `docs/03-pain-points.md` (PAIN-## IDs). Role definitions are canonical from `README.md §C` and are the authoritative input to `docs/10-permission-role-matrix.md`. The self-registration rejection is formalized in `adr/ADR-0004`. Stream-scope ABAC rules are specified further in `docs/10-permission-role-matrix.md` and `docs/07-functional-requirements.md`.*

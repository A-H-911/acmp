...
Capabilities are organized by canonical module (README §B) and phase. Phase definitions follow `docs/36-roadmap.md`:
...
| 3 | Global RBAC role assignment: Chairman, Secretary, Member, Reviewer, Auditor, Administrator, Submitter, Guest/Presenter | Membership | 1 | README §C; full matrix in `docs/10` |
...
| 17 | Topic aging indicator (visual warning when topic has been in same status beyond SLA thresholds by urgency) | Topics | 1 | brief §6.3; urgency SLA table in `docs/09` |
...
| 89 | Notification event catalog: topic status changes, vote open/close, action reminders, MoM published, action overdue, escalation | Notifications | 1 | brief §6.17; `docs/29` |
...
| 98 | KPI dashboard: committee health indicators (aging backlog, decision turnaround, action SLA compliance) | Reporting | 3 | `docs/28` |
| 99 | Full-text search across topics, decisions, ADRs, wiki, MoMs (SQL FTS) | Search&Traceability | 1 | ADR-0011 |
| 100 | Typed traceability graph: navigate upstream/downstream links from any artifact | Search&Traceability | 1 | ADR-0008; `docs/30` |
...
| 103 | Append-only audit log for all state transitions on governed entities; immutable | Audit&Records | 1 | ADR-0009; `docs/26` |
...
| D-02 | Webex recording/transcript retrieval and storage | Manual (secretary pastes link/uploads) in Phase 1; automated via Webex API in Phase 2 | Webex Assistant enabled org-wide and API access confirmed (OQ ref `docs/42`) |
...
- In-scope capabilities → `docs/07-functional-requirements.md` (each capability maps to ≥1 FR)
- Out-of-scope rationale → `docs/05-product-vision-and-principles.md §Anti-Goals` (anti-goals stated there; operationalized here)
- Phase definitions → `docs/36-roadmap.md`
- Open decisions affecting scope → `docs/42-open-decisions.md` (OQ items referenced above)
- Settled technology decisions constraining scope → `README.md §A` and `adr/ADR-0001` through `AD
...
| AC-014 | SoD-2 | Partial | MinutesHandlerTests (sole-author approval allowed + flagged; different approver clears the flag) + MinutesApiTests (sole-author publish → ApprovedBySoleAuthor) | P7c: soft SoD-2 — sole-author approval allowed, emits `MinutesApprovedBySoleAuthor` + sets the `ApprovedBySoleAuthor` warning flag; live UI indicator → P7d/P17 |
...
| AC-036 | MoM | Partial | MinutesOfMeetingTests (supersede from Approved/Published + no public setters) + MinutesHandlerTests (v2 under same key, prior Superseded + back-link, readable) + MinutesApiTests (supersede 201 v2, prior back-link) | P7c: version-preserving supersede (same MIN key, Version++), prior immutable + linked; live HTTP/UI → P7d/P17 (G-TRACE) |
| AC-037 | MoM | Partial | MinutesOfMeetingTests (InReview→Draft) + MinutesHandlerTests (change-request → Draft + author notified) + MinutesApiTests (request-changes → Draft) | P7c: change-request returns to Draft, targeted author notification; doc 12 §6 gap (edge omitted) noted; live UI → P7d/P17 |
| AC-038 | MoM | Partial | MinutesOfMeetingTests (Approved→Published) + MinutesHandlerTests (publish fans out per active member + deep link + audit) + MinutesApiTests (draft→submit→approve→publish→Published) | P7c: publish seals + notifies all members (deep link `/meetings/{key}/minutes`); AC-038's single-step prose maps to Approve+Publish (5-state); live UI → P7d/P17 |

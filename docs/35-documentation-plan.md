...
| **arc42 architecture doc** | `/docs/arc42/` (generated from `/docs/15-architecture.md`) | Markdown | Lead engineer | Architecture decision change (ADR update) |
...
| User guide EN/AR diverges | AR guide update is part of Definition of Done for any user-visible feature (see `docs/44-definition-of-done.md`) |
...
3. `docs/15-architecture.md` — macro-architecture, module boundaries.
...
7. `docs/33-containerization-and-deployment.md` — how to run the stack locally.
...
|---|---|
| `docs/15-architecture.md` | → `docs/arc42/` (restructured per arc42) |
| `adr/ADR-0001 … ADR-0012` | → `adr/ADR-0001 … ADR-0012` (copied verbatim, reviewed) |
| `docs/34-repository-structure.md` | → Scaffolded by Claude Code agent as the actual repo |
| `docs/33-containerization-and-deployment.md` | → `deploy/README.md` + Dockerfiles + Compose files |
| `docs/31-testing-strategy.md` | → `docs/testing-strategy.md` + encoded as test patterns in code |
| `docs/35-documentation-plan.md` | → `docs/documentation-plan.md` + triggers CLAUDE.md and per-module README templates |
...
| `docs/40-acceptance-criteria.md` | → Encoded as test `DisplayName` tags in test projects |
| `docs/07-functional-requirements.md` | → Referenced as `FR-###` in backlog/issue tracker |

---

## Traceability

Links: `docs/34-repository-structure.md` (directory layout implementing this plan) · `docs/32-devsecops-plan.md` (CI link-check and i18n-check stages) · `docs/20-keystone-analysis-and-integration.md` (Keystone gate G-COMPLETE, G-TRACE — applied to planning docs) · `docs/19-tarseem-analysis-and-integration.md` (diagram JSON spec conventions) · `adr/ADR-0006-tarseem-diagram-engine.md` · `../README.md` §A (ADR-0009 immutability philosophy applied to ADR lifecycle).

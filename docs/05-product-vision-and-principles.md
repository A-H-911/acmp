...
Each goal is measurable at the end of Phase 1 MVP acceptance testing (see `docs/40-acceptance-criteria.md`).
...
Every unchecked assumption is documented as `ASM-###` in `docs/41-raid.md`. Every unresolved product or technical question is an `OQ-###` in `docs/42-open-decisions.md`. Nothing remains a silent assumption. If a feature depends on an open decision (e.g., blob storage provider), that dependency is explicit and the feature is not committed as "done" until the decision is resolved.
...
- Vision and goals → `docs/00-executive-summary.md`, `docs/01-organization-and-problem.md`, `docs/03-pain-points.md`
- Principles → `README.md §Guiding principles` (expanded here); `adr/ADR-0001` through `ADR-0012` (each principle maps to ≥1 settled ADR)
- Anti-goals → `docs/06-scope-and-out-of-scope.md §Out-of-scope` (anti-goals are repeated there with implementation-level detail)
- Measurable goals → `docs/40-acceptance-criteria.md` (acceptance criteria operationalize each goal)
- Product goals G-1–G-5 → `docs/28-metrics-and-kpi-catalog.md` (KPIs that track goal achievement post-release)

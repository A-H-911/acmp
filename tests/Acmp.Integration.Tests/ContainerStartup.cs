...
internal static class ContainerStartup
{
    ...
    // DW-085 (DEC-078 d2). The image BUILD is the other unbounded await on this path: FtsImage built
    ...
    // sit unnoticed.
    //
    // ⚠ SINCE WBS-41.2 FtsImage PULLS the pinned image instead, and this same budget bounds that `docker pull`
    // (it stands on the same path to the first container); the build below runs only under ACMP_FTS_BUILD=1.
    // The measurements that follow are of the BUILD, and were not re-taken for the pull.
    //

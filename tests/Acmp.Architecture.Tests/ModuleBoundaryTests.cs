...
public class ModuleBoundaryTests
{
    ...
    [Fact] // Research (P15a) is self-contained: it depends only on Acmp.Shared contracts — never another
           // module's assemblies (ADR-0001). SourceTopicId / LinkedTopicId are opaque value ids; the
           // Mission→Topic / Finding→Decision / Recommendation→Action graph edges are P15c, not here.
    public void Research_should_not_depend_on_other_modules()
    {
        var result = Types.InAssemblies(new[] { ResearchDomain, ResearchApp })
            .Should().NotHaveDependencyOnAny(
                "Acmp.Modules.Topics.Domain", "Acmp.Modules.Topics.Application", "Acmp.Modules.Topics.Infrastructure",
                "Acmp.Modules.Membership.Domain", "Acmp.Modules.Membership.Application", "Acmp.Modules.Membership.Infrastructure",
                "Acmp.Modules.Meetings.Domain", "Acmp.Modules.Meetings.Application", "Acmp.Modules.Meetings.Infrastructure",

# ADR-0018: Optimistic Concurrency via RowVersion (implements docs/16 §1.5)

- Status: Accepted
- Date: 2026-06-30
- Deciders: Architecture Committee execution (operator-confirmed, 2026-06-30)
- Implements: `docs/16-data-architecture-and-model.md` §1.5
- Resolves: OQ-043 (`docs/42-open-decisions.md`)

## Context and Problem Statement

`docs/16` §1.5 has always stated that every **mutable aggregate root** carries a `RowVersion ROWVERSION` column so EF Core optimistic concurrency turns a stale write into a `DbUpdateConcurrencyException`, which the API maps to **HTTP 409** (`docs/15-architecture.md` §7.4). The forensic rebuild review (rebuild-findings DI-02 / OQ-043) found the backstop **did not exist in code**: no `IsRowVersion()` / `[Timestamp]` / concurrency token on any entity, and no `RowVersion` column in any migration. Documentation and code disagreed; CLAUDE.md and guardrail #11 require we fix one or raise an ADR rather than let them drift. OQ-043 framed the choice: **(a)** implement the token to make the doc true, or **(b)** amend `docs/16` to last-writer-wins.

## Decision Drivers

- The system of record for committee governance should not silently lose an update when two users edit the same aggregate; surfacing a 409 ("reload and retry") is the auditable behavior `docs/16`/`docs/15` already specify.
- `docs/16` was authored in detail assuming optimistic concurrency (the §1.5 paragraph, the §2 table conventions, and per-table `RowVersion` columns). Option (a) makes a coherent design true; option (b) would require scrubbing the claim from many places and weakening a settled model.
...
2. **Last-writer-wins** — drop the rowversion claim from `docs/16` §1.5 + the §2 conventions + every table row + `docs/15` §7.4, and accept silent overwrites at this scale.
...
Applied in v1 to the **built** mutable aggregate roots: `Topic` (Topics), `Meeting` and `Agenda` (Meetings), and `CommitteeMember` (Membership). As further mutable roots from `docs/16` are built (e.g. `MinutesOfMeeting`, `Document`, `Decision`, `Action`), each carries `RowVersion` at build time per `docs/16` §1.5 — this ADR is the standing rule.
...
- Good: stale concurrent writes fail loud (409) instead of silently overwriting; `docs/16` §1.5 and `docs/15` §7.4 are now true in code; the S5 concurrency invariant is testable.
...
- Implements `docs/16-data-architecture-and-model.md` §1.5; status codes per `docs/15-architecture.md` §7.4.
- Resolves OQ-043 (`docs/42-open-decisions.md`); recorded as R-27 in §A.
